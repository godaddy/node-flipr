...
describe('Flipr', () => {
  ...
  describe('validateConfig', () => {
    it('calls source\'s validateConfig, passing along rules', async () => {
      const validationResult = [];
      const rules = ['some', 'rules'];
      const validateConfig = jest.fn();
      validateConfig.mockReturnValue(validationResult);
      const flipr = new Flipr({
        source: {
          validateConfig,
        },
      ...
      const result = await flipr.validateConfig();
      await expect(validateConfig).toHaveBeenCalledWith(rules);
